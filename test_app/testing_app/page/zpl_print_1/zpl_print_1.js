frappe.pages['zpl_print-1'].on_page_load = function(wrapper) {
...
page.main.css({
	...
	$('<div style="display:none" id="qz-status" bgcolor="#FFF380">\
      ...
      <input type="button" id="list_ports" onClick="listSerialPorts()" value="List Serial Ports" /><br />\
        ...
        <hr /><h2>Misc</h2>\
        <input type="button" onClick="listNetworkInfo()" value="List Network Info" /><br /></div>').appendTo(page.main);

	window["deployQZ"] = typeof(deployQZ) == "function" ? deployQZ : this.deployQZApplet;
	deployQZ();
	page.delivery_note = wrapper.page.add_field({fieldname: "delivery_note", fieldtype: "Link",
		options: "Delivery Note", label: __("Delivery Note No")});

	page.detect_printer = wrapper.page.add_field({fieldname: "detect_printer", fieldtype: "Button",
		label: __("Detect Printer")}).$input.
		click(function(){
			findPrinter()
	});
...
PrintZPLData = Class.extend({
	init: function(wrapper, page){
		var me = this;
		this.wrapper = wrapper;
		this.delivery_note = page.delivery_note.$input.val();
		this.prepare_zpl_data();
		this.print_data();
	},
	prepare_zpl_data: function(){
		var me = this;
	},
	print_data: function(){
		printZPL()
